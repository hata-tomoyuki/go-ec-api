...
// mockQuerier は Querier interface の手動モック。
// テストで使うメソッドだけ実装し、それ以外は panic する。
type mockQuerier struct {
	// 各メソッドの振る舞いをテストごとに差し替えるための関数フィールド
...
func (m *mockQuerier) FindAddressById(ctx context.Context, id int32) (repo.Address, error) {
	return m.findAddressByIdFn(ctx, id)
}
...
func (m *mockQuerier) CreateAddress(ctx context.Context, arg repo.CreateAddressParams) (repo.Address, error) {
	return m.createAddressFn(ctx, arg)
}
...
// テスト用のヘルパー: ダミーの Address を生成
func newTestAddress(id int32, userID int64) repo.Address {
	return repo.Address{
		ID:        id,
		UserID:    userID,
		Street:    "123 Test St",
